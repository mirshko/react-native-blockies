...
var Canvas = createReactClass({
  propTypes: {
    ...
    blockies: PropTypes.string.isRequired
  },

  componentWillReceiveProps(newProps) {
    if (newProps.blockies != this.props.blockies) {
      this.refs.webview.injectJavaScript(`generate('${newProps.blockies}', ${newProps.size})`);
    }
  },
    ...
    return (
      <View style={[{ backgroundColor: 'transparent' }, this.props.style]}>
        <WebView
          ...
          scrollEnabled={false}
          injectedJavaScript={`generate('${this.props.blockies}', ${this.props.size})`}
        />
      </View>
    );
  }
